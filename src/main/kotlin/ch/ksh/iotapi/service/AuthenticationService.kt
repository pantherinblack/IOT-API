...
class AuthenticationService {
    ...
    fun login(
        ...
        if (AuthenticationHandler.getInstance().isValidUser(userName,password)) {
            ...
            header.add(HttpHeaders.SET_COOKIE, String.format("%s=%s; Max-Age=%d; Path=%s", userNameCookie.name, userNameCookie.value, userNameCookie.maxAge, "/"))
            header.add(HttpHeaders.SET_COOKIE, String.format("%s=%s; Max-Age=%d; Path=%s", passwordCookie.name, passwordCookie.value, passwordCookie.maxAge, "/"))
